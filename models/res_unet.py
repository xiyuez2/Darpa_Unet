...
class Resnet_Unet(nn.Module):
    ...
    """
    def __init__(self, BN_enable=True, resnet_pretrain=False,n_channels=6,n_classes=2):
        super().__init__()
        self.BN_enable = BN_enable
        # encoder部分
        ...
        self.firstconv = nn.Conv2d(in_channels=n_channels, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        if self.BN_enable:
            self.final = nn.Sequential(
                ...
                nn.Conv2d(in_channels=32, out_channels=n_classes, kernel_size=1),
                nn.Sigmoid()
                )
        else:
            self.final = nn.Sequential(
