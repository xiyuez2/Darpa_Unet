...
# import numpy as np
from .spixel_utils import poolfeat
from .spixel_fcn import SpixelNet
...
class SegmentationModel(pl.LightningModule):
    def __init__(self, args=None):
        ...
        if model_name == "Unet":
            ...
            self.model = SPixelUnet(n_channels=n_channels,n_classes=2, spixel_ckpt=sp_ckpt,spixel_sz=sp_sz, spixel_pretrain=sp_pretrain)
        else:
...
class SP_Down(nn.Module):
    """Downscaling with superpixel pooling then Double conv"""
    def __init__(self, in_channels, out_channels, sp_size=2):
        super().__init__()
        self.conv = DoubleConv(in_channels, out_channels)
        self.kernel_sz = sp_size

    def forward(self, x, prob_in):
        x = poolfeat(x,prob_in, self.kernel_sz, self.kernel_sz)
        return self.conv(x)

class SP_Compress(nn.Module):
    """Compress Channel to 3 for superpixel input"""
    def __init__(self, in_channels, out_channels=3):
        super().__init__()
        self.comporess_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), bias=False),
            nn.BatchNorm2d(out_channels)
        )
    
    def forward(self, x):
        return self.comporess_conv(x)
...
class SPixelUnet(nn.Module):
    def __init__(self, n_channels, n_classes, spixel_ckpt, spixel_sz=2, spixel_pretrain=True, training=True, bilinear=False):
        ...
        # superpixel init
        self.spixel_model = SpixelNet()
        if spixel_pretrain:
            self.spixel_model.load_state_dict(torch.load(spixel_ckpt)['state_dict'])
        self.kernel_sz = spixel_sz

        self.inc = (DoubleConv(n_channels, 64))
        self.down1 = (SP_Down(64, 128))
        self.down2 = (SP_Down(128, 256))
        self.down3 = (SP_Down(256, 512))
        factor = 2 if bilinear else 1
        self.down4 = (SP_Down(512, 1024 // factor))
        self.up1 = (Up(1024, 512 // factor, bilinear))
        self.up2 = (Up(512, 256 // factor, bilinear))
        self.up3 = (Up(256, 128 // factor, bilinear))
        ...
        # compress channel for spixel_fcn output
        self.compress1 = (SP_Compress(64,3))
        self.compress2 = (SP_Compress(128,3))
        self.compress3 = (SP_Compress(256,3))
        self.compress4 = (SP_Compress(512,3))
    ...
    def forward(self, map, legend):
        ...
        x = torch.cat((map, legend),axis=1) # torch.Size([32, 6, 256, 256]) B,C, H, W
        # print("[DEBUG] Input size: ", x.size(),map.size(),legend.size())
        x1 = self.inc(x)    # x1 shape: (32,)
        # print("[DEBUG] x1:", x1.shape)
        
        x1_comp = self.compress1(x1)
        # print("[DEBUG] x1_comp:", x1_comp.shape)
        prob1 = self.spixel_model(x1_comp)
        x2 = self.down1(x1, prob1)
        # print("[DEBUG] x2:", x1.shape)
        
        x2_comp = self.compress2(x2)
        # print("[DEBUG] x2_comp:", x1_comp.shape)
        prob2 = self.spixel_model(x2_comp)
        x3 = self.down2(x2, prob2)
        # print("[DEBUG] x3_comp:", x1_comp.shape)
        
        x3_comp = self.compress3(x3)
        prob3 = self.spixel_model(x3_comp)
        x4 = self.down3(x3, prob3)
        
        x4_comp = self.compress4(x4)
        prob4 = self.spixel_model(x4_comp)
        x5 = self.down4(x4, prob4)
